...
from hummingbot.connector.exchange.deribit.deribit_order_book import DeribitOrderBook
...
class DeribitAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        ...
        try:
            ...
            for trading_pair in self._trading_pairs:
                symbol = await self._connector.exchange_symbol_associated_to_pair(trading_pair=trading_pair)
                trade_params.append(f"trades.{symbol.lower()}.100ms")
                depth_params.append(f"book.{symbol.lower()}.none.10.100ms")
            payload = {
                "jsonrpc": "2.0",
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_msg: OrderBookMessage = DeribitOrderBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
    ...
    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "result" not in raw_message:
            ...
            trade_message = DeribitOrderBook.trade_message_from_exchange(
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "result" not in raw_message:
            trading_pair = await self._connector.trading_pair_associated_to_exchange_symbol(symbol=raw_message["s"])
            order_book_message: OrderBookMessage = DeribitOrderBook.diff_message_from_exchange(
